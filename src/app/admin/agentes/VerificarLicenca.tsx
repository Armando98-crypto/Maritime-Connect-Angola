...
export function VerificarLicenca({ userId, verificada }: VerificarLicencaProps) {
  ...
  return (
    <div className="flex flex-col items-end gap-2">
      ...
      {verificada ? (
        <Button variante="secundario" tamanho="sm" aCarregar={aCarregar} onClick={() => actualizar(false)}>
          Marcar como pendente
        </Button>
      ) : (
        <Button variante="primario" tamanho="sm" aCarregar={aCarregar} onClick={() => actualizar(true)}>
          Verificar licença
        </Button>
      )}
    </div>
