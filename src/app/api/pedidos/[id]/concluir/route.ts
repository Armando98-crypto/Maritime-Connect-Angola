...
export async function POST(
  ...
  if (sessao.user.papel !== "ARMADOR") {
    return NextResponse.json(
      { erro: "Só armadores podem concluir pedidos." },
      { status: 403 }
    );
  }
  ...
  try {
    await concluirPedido(pedidoId, sessao.user.id);
    ...
    if (
      erro instanceof SemPermissaoPedidoError ||
      erro instanceof PedidoNaoConcluivelError
    ) {
      return NextResponse.json({ erro: erro.message }, { status: 409 });
    }
    console.error("Erro inesperado ao concluir pedido:", erro);
    return NextResponse.json(
      { erro: "Não foi possível concluir o pedido. Tente novamente." },
      { status: 500 }
    );
  }
}